...
INSTANCEID="$(aws ec2 run-instances --block-device-mappings "DeviceName=/dev/sda1, Ebs={VolumeSize=30}" --image-id "$AMIID" --key-name yourkey --instance-type t2.micro --security-group-ids "$SGID" --subnet-id "$SUBNETID" --query "Instances[0].InstanceId" --output text)"
...
echo "ssh -i yourkey.pem ec2-user@$PUBLICNAME"
